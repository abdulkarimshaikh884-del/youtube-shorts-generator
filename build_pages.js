...
const NAV = [
  ...
  { href: "/editor", label: "Editor", key: "editor",
    icon: '<rect x="2.5" y="5" width="14" height="14" rx="3"/><path d="M16.5 10l5-3v10l-5-3z"/>' },
  { href: "/pricing", label: "Pricing", key: "pricing",
    icon: '<path d="M3 10.5V6a2 2 0 0 1 2-2h4.5L21 15.5 15.5 21 4 9.5"/><circle cx="7.5" cy="7.5" r="1.4"/>' },
  { href: "/contact", label: "Help &amp; Feedback", key: "contact",
    icon: '<path d="M4 4h16v13H8l-4 4z"/>' }
];
...
const indexPage = {
  ...
  body: `    <main class="sh-home">
      <section class="sh-hero">
        <span class="sh-eyebrow">✦ CSS Motion-Graphics Engine</span>
        <h1>AI Motion Graphics <br><span class="sh-grad">for Shorts &amp; Reels</span></h1>
        <p class="sh-sub">Describe what you want to imagine or pick a motion graphic template below to customize in the editor.</p>
      </section>
      ...
      <form class="sh-composer" id="composer" action="/editor" method="GET">
        <div class="sh-ctop">
          ...
          <textarea id="composerPrompt" name="topic" rows="2" maxlength="600"
            placeholder="Describe an animation... e.g. a glass pricing card that flips to reveal ₹99, dark with one green accent"></textarea>
        </div>
        <div class="sh-cbar">
          <input type="file" id="composerImg" accept="image/png,image/jpeg,image/webp,image/gif" class="ed-sr">
          <label class="sh-imgbtn" for="composerImg" title="Attach an image to animate">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><rect x="3" y="4" width="18" height="16" rx="3"/><circle cx="8.5" cy="9.5" r="1.6"/><path d="M4 17l5-4.5 4 3.5 3-2.5 4 3.5"/></svg>
            <span>Attach image</span>
          </label>
          <button class="sh-imgchip" type="button" id="composerImgClear" hidden>
            <span id="composerImgName">image</span> <b>×</b>
          </button>
          <select class="sh-model-sel" id="qualitySelect" name="quality">
            <option value="mini" selected>Free</option>
            <option value="pro">Pro</option>
            <option value="max">Pro Max</option>
          </select>
          <span class="sh-cost">5 credits</span>
          <button class="sh-go" type="submit">Create in Editor <span aria-hidden="true">→</span></button>
        </div>
      </form>

      <section class="sh-gallery-sec" id="templates">
        <header class="sh-ghead">
          <div class="sh-gtitle">
            <span class="sh-eyebrow">✦ Ready-Made Library</span>
            <h2>AI Motion Graphics Video Generator Templates</h2>
            <p>Pick any template to customize text, typography, duration, and colors in the Studio.</p>
          </div>
          <div class="sh-chips" id="filters"></div>
        </header>
        <div class="sh-gallery" id="gallery"></div>
      </section>
